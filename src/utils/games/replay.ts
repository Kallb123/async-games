...
export async function buildTimeline(
    ...
    const applyCommands = async (
        ...
        for (const raw of rawCommands) {
            ...
            // command fed in: a game can answer one with a follow-up (see
            // ICommandOutcome.followUpCommand), and that follow-up gets its own
            // step here rather than being folded into the trigger's. Follow-ups
            // are regenerated by this very replay rather than read back from
            // commandHistory, so the timeline a player steps through has the
            // same shape as the turn they lived through.
            const { outcome, gameOver } = await runCommandChain(state, gameType, command, (applied, appliedOutcome) => {
