 ...
 * colour) — and the role the seat is playing.
 */
export default function BannedIsletHands({ playerStates, userIdList, turnOrder, myUserId, activeUserId }: BannedIsletHandsProps) {
    const seats: PlayerHandSeat[] = Object.values(playerStates).map(ps => ({
        ...
        cards: ps.hand.map((card, index) => (
            <div className="ag-hand-card ag-hand-card--named" key={`${card}-${index}`} title={cardName(card)}>
                <span className="ag-hand-card-emoji" aria-hidden="true">{cardGlyph(card)}</span>
                <span className="ag-hand-card-name">{cardName(card)}</span>
            </div>
        )),
