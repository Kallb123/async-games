...
- `src/components/ui/` — presentational primitives:
  ...
  message. Never hand-roll an `ag-section-head` again. A dismissible `.ag-log`
  panel — the chat thread, the turn-history log, the GIF picker — opens with a
  `PanelHead`: its title, its dim subtitle and its own ✕, so the next one can't
  forget the label on that button. `ChatGif` is one GIF
