...
| `TENOR_API_KEY` | its own key, so the GIF picker's quota and analytics are production's | a different key, or unset — with none the picker reports "GIFs unavailable" and the rest of chat is unaffected |
