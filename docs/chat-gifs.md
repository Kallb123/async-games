...
|---|---|---|---|---|
| **A. In-app picker** | a URL + an id from a GIF catalogue | somebody else's CDN | the provider's content filter | every platform, identically |
...
frame is acceptable. Keyboard insertion stores a *file*: it needs a blob store
...
- **Store the URL *and* the id.** The URL because a provider's URL shape is not
  ours to reconstruct and a stored message must still render in a year; the id
  because the share ping (§5) needs it and because it is the stable handle if we
  ever want to re-resolve a dead URL.
...
## 4. Validation: the host allowlist is the whole gate

`src/utils/chat.ts` — the module the composer and the POST route already share
so their limits can't drift — gains `normaliseAttachment(value): IChatAttachment | null`
and relaxes `normaliseMessage` so an empty `text` is legal *when an attachment
is present*. The POST route's check becomes "a valid message, or a valid
attachment, or 400".

**The client sends a URL, so the server must not take it on trust.** This is
squarely the locksmith's boundary — a client sending a field it shouldn't
control. An unchecked `url` on a chat message means any player can make every
other player's browser issue a GET to any host they choose, which hands that
host each opponent's IP and user-agent, works as a read receipt, and can point
at a decompression bomb or a malformed image instead of a GIF. None of that
needs a bug in our code; it is just what an `<img src>` does.

So `normaliseAttachment` rejects anything that isn't:

- `provider` in the known set;
- `url` and `stillUrl` parsing as `https:` URLs whose hostname is on a
  **hard-coded allowlist** (`media.tenor.com` and the provider's siblings) —
  matched as a whole host, never `endsWith`, which `evil-media.tenor.com.example`
  defeats;
- no credentials, no port, no fragment, and a bounded length;
- `width`/`height` positive integers under a sane cap;
- `alt` a string, trimmed, length-capped, stored as text (the thread renders it
  through React's escaping; nothing in chat goes near `dangerouslySetInnerHTML`);
- `mediaId` matching the provider's own id shape.

This mirrors `src/utils/reactions.ts` exactly: a fixed server-side whitelist
shared with the picker, so the two cannot disagree. And it is the same
allow-list-a-host instinct `next.config.mjs` already applies to `img.clerk.com`
for avatars.
...
`img-src` is the belt to this braces and should list the same hosts from the
same constant.
...
key must not ship to the client.
...
- `contentfilter=high` and `media_filter` set so the upstream returns only the
  ...
  or the filter becomes a client-supplied field too.
- `AbortSignal.timeout(…)` on the upstream fetch, and a provider failure
  ...
  same "a push failure never undoes the message" instinct the chat POST already
  has.
- The response is **mapped down** to the `IChatAttachment` fields plus a
  thumbnail — never the provider's JSON passed through, which would leak
  whatever else it carries and couple our client to their schema.
...
Tenor also asks for a `registershare` ping when a result is actually sent. That
belongs on the POST route inside its existing `after()` block, next to the push
fan-out and guarded the same way: it is analytics, and it must never be able to
fail a message that already saved.
...
1. **The host allowlist (§4) is the moderation boundary.** Arbitrary URLs would
   be the version of this feature that genuinely needs moderating; a fixed
   provider is not.
2. **`contentfilter=high` (§5)** does the bulk of it upstream, and is pinned
   server-side where a client can't relax it.
...
1. **Model + validation.** `IChatAttachment` on `ChatMessageData`,
   `normaliseAttachment` and the relaxed `normaliseMessage` in
   `src/utils/chat.ts`, the host allowlist constant, unit tests on the
   rejections (wrong host, `http:`, a subdomain-suffix trick, missing
   dimensions, both-empty). No route changes yet — nothing can send one.
2. **The POST route accepts one.** `attachment` read through the validator,
   400 on a bad one, the existing membership gate and rate limit unchanged, and
...
4. **The search route.** `/api/gif/search`, auth + rate limit + timeout +
   mapped response + cache header, and a route test for the unauthenticated,
   over-limit and upstream-failed cases.
